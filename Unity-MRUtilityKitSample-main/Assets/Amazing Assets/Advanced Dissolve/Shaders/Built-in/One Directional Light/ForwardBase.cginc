...
#ifndef ADVANCED_DISSOLVE_FORWARDBASE_CGINC
#define ADVANCED_DISSOLVE_FORWARDBASE_CGINC
...
//Fragment//////////////////////////////////////////////////////////////
float4 frag (vOutput i, fixed facing : VFACE) : SV_Target
{
...
//Advanced Dissolve////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    float4 dissolveBase = 0;
        ...
        dissolveBase = tex2D(_MainTex, mainUV);
        dissolveBase.rgb *= _Color.rgb;
    #endif

	ADVANCED_DISSOLVE_SETUP_CUTOUT_SOURCE_USING_WS(i, dissolveBase, i.positionWS.xyz, i.normal.xyz)
	...
	float dissolveBlend = AdvancedDissolveAlbedoEmission(cutoutSource, dissolveBase, dissolveAlbedo, dissolveEmission, mainUV);
...
//Advanced Dissolve/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	float4 mainTex = tex2D(_MainTex, mainUV);
		...
		//Advanced Dissolve
		#if defined(_AD_STATE_ENABLED)
			AdvancedDissolveCalculateAlphaAndClip(cutoutSource, retColor.a, cutout);
		#endif


		clip(retColor.a - cutout);	
	#endif		
	...
	//Vertex Color
	retColor *= lerp(1, i.color, _IncludeVertexColor);
	...
	#if defined(_REFLECTION)
		#ifdef _NORMALMAP
			float4 reflTex = texCUBE( _ReflectionCubeMap, i.refl.xyz * face + bumpNormal) * _ReflectionColor;
		#else
			float4 reflTex = texCUBE( _ReflectionCubeMap, i.refl.xyz * face) * _ReflectionColor;
		#endif
	...
	#ifdef _RIM
		#ifndef LIGHTMAP_OFF
			float rimFresnel = 1 - saturate(dot (normalize(i.normal.xyz), normalize(i.viewDir.xyz) * face) + _RimBias);			
		#else
			float rimFresnel = 1 - saturate(dot (normal, normalize(i.viewDir.xyz) * face) + _RimBias);			
		#endif
...
//Advanced Dissolve/////////////////////////////////////////
#if defined(_AD_STATE_ENABLED)
    retColor.rgb = lerp(retColor.rgb, dissolveAlbedo, dissolveBlend);
    retColor.rgb += dissolveEmission * dissolveBlend;
#endif
